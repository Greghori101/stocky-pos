...
class PosReservationController extends BaseController
{
            ...
            try {
                ...
                if($request['amount'] > 0){
                    if ($request->payment['Reglement'] == 'credit card') {
                        ...
                        $PaymentReservation->Reglement = $request->payment['Reglement'];
                        $PaymentReservation->montant   = $request['amount'];
                        $PaymentReservation->change    = $request['change'];
                        $PaymentReservation->notes     = $request->payment['notes'];
    ...
    public function get_draft_reservations(request $request)
    {
        ...
        // Check If User Has Permission View  All Records
        $draft_reservations = DraftReservation::with('client', 'warehouse','user')
        ...
        $totalRows = $draft_reservations->count();
        if($perPage == "-1"){
            $perPage = $totalRows;
        }
        
        $drafts = $draft_reservations->offset($offSet)
            ->limit($perPage)
            ->orderBy($order, $dir)
            ->get();
     ...
     public function remove_draft_reservation(Request $request, $id)
     {
        $this->authorizeForUser($request->user('api'), 'Reservations_pos', Reservation::class);
         ...
         \DB::transaction(function () use ($id, $request) {
             ...
             $role = Auth::user()->roles()->first();
             $view_records = Role::findOrFail($role->id)->inRole('record_view');
             $draft = DraftReservation::findOrFail($id);
    ...
    public function submit_reservation_from_draft(Request $request)
    {
        ...
        $draft = DraftReservation::findOrFail($request['draft_reservation_id']);
        if($draft){
    ...
    //--------------------- data_draft_convert_reservation ------------------------\\
    ...
    public function data_draft_convert_reservation(Request $request, $id)
    {
        ...
        $draft_reservation_data = DraftReservation::with('details.product.unitReservation')->where('deleted_at', '=', null)->findOrFail($id);
        ...
        if ($draft_reservation_data->client_id) {
            if (Client::where('id', $draft_reservation_data->client_id)->where('deleted_at', '=', null)->first()) {
                $reservation['client_id'] = $draft_reservation_data->client_id;
                $client_name = $draft_reservation_data['Client']->name;
            } else {
        ...
        if ($draft_reservation_data->warehouse_id) {
            if (Warehouse::where('id', $draft_reservation_data->warehouse_id)->where('deleted_at', '=', null)->first()) {
                $reservation['warehouse_id'] = $draft_reservation_data->warehouse_id;
            } else {
        ...
        $reservation['tax_rate'] = $draft_reservation_data->tax_rate;
        $reservation['TaxNet']   = $draft_reservation_data->TaxNet;
        $reservation['discount'] = $draft_reservation_data->discount;
        $reservation['shipping'] = $draft_reservation_data->shipping;
        $GrandTotal = $draft_reservation_data->GrandTotal;

        $detail_id = 0;
        foreach ($draft_reservation_data['details'] as $detail) {
            ...
            if ($detail->product_variant_id) {
                $item_product = product_warehouse::where('product_id', $detail->product_id)
                    ...
                    ->where('warehouse_id', $draft_reservation_data->warehouse_id)
                    ->first();
                ...
                $item_product = product_warehouse::where('product_id', $detail->product_id)
                    ->where('deleted_at', '=', null)->where('warehouse_id', $draft_reservation_data->warehouse_id)
    ...
    public function GetProductsByParametre(request $request)
    {
        ...
        $product_warehouse_data = product_warehouse::where('warehouse_id', $request->warehouse_id)
            ->with('product', 'product.unitReservation')
            ->where('deleted_at', '=', null)
        ...
        foreach ($product_warehouse_data as $product_warehouse) {
            ...
            if($product_warehouse['product']['unitReservation']){

                if ($product_warehouse['product']['unitReservation']->operator == '/') {
                    $item['qte_reservation'] = $product_warehouse->qte * $product_warehouse['product']['unitReservation']->operator_value;
                    $price = $product_price / $product_warehouse['product']['unitReservation']->operator_value;
                ...
                $item['qte_reservation'] = $product_warehouse['product']->type!='is_service'?$product_warehouse->qte:'---';
                $price = $product_price;
            }

            $item['unitReservation'] = $product_warehouse['product']['unitReservation']?$product_warehouse['product']['unitReservation']->ShortName:'';
    ...
    public function GetELementPos(Request $request)
    {
      ...
      public function getNumberOrderDraft()
      {
          ...
          $last = DB::table('draft_reservations')->latest('id')->first();
