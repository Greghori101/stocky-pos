		...
		<div class="container-fluid p-0 app-admin-wrap layout-sidebar-large clearfix" id="pos">
			...
			<b-row v-if="!isLoading">
					...
					<b-card no-body class="card-order">
						<div class="main-header">
							...
							<div class="header-part-right">
								<!-- Dashboard -->
								<router-link class="i-Back header-icon d-sm-inline-block" to="/app/dashboard" title="Dashboard"> </router-link>

								<!-- Full screen toggle -->
								<i style="color: #8b5cf6" title="Full screen" class="i-Full-Screen header-icon d-none d-sm-inline-block" @click="handleFullScreen"></i>
									...
									<b-dropdown id="dropdown" text="Dropdown Button" class="m-md-2" toggle-class="text-decoration-none" no-caret right variant="link">
										...
										>
											<div class="menu-icon-grid">
												<a @click="SetLocal('en')"> <i title="en" class="flag-icon flag-icon-squared flag-icon-gb"></i> English </a>
												<a @click="SetLocal('fr')">
								...
								<div class="dropdown">
									<b-dropdown id="dropdown-1" text="Dropdown Button" class="m-md-2 user col align-self-end" toggle-class="text-decoration-none" no-caret variant="link" right>
										...
										<div class="dropdown-menu-left" aria-labelledby="userDropdown">
											...
											<router-link v-if="currentUserPermissions && currentUserPermissions.includes('setting_system')" to="/app/settings/System_settings" class="dropdown-item">{{
												$t("Settings")
											}}</router-link>
															...
															<tr v-for="(detail, index) in details" :key="index">
																<td>
																	...
																	<i
																		...
																		class="i-Edit text-success cursor-pointer"
																	></i>
																</td>
									...
									<b-row>
										...
										<b-col lg="12" md="12" sm="12" v-show="detail.is_imei">
											<b-form-group :label="$t('Add_product_IMEI_Serial_number')">
												<b-form-input label="Add_product_IMEI_Serial_number" v-model="detail.imei_number" :placeholder="$t('Add_product_IMEI_Serial_number')"></b-form-input>
											</b-form-group>
										</b-col>
								...
								<div @click="Check_Product_Exist(product, product.id)" v-for="product in products" class="card o-hidden bd-highlight m-1">
									...
									<div class="flex-grow-1 d-bock">
										<div class="card-body align-self-center d-flex flex-column justify-content-between align-items-lg-center">
											...
											<p v-if="product.product_type != 'is_service'" class="m-0 text-muted text-small w-15 w-sm-100 d-none d-lg-block item-badges">
					...
					<div class="px-3 py-2">
						...
						<b-row>
							<div class="col-md-12 flex-row flex-wrap bd-highlight list-item mt-2">
								<div @click="getAllCategory()" :class="{ 'brand-Active': category_id == '' }" class="card bd-highlight m-1">
								...
								<tbody>
									<tr v-for="detail_invoice in invoice_pos.details">
										<td colspan="3">
											...
											<br />
											<span
												>{{ formatNumber(detail_invoice.quantity, 2) }} {{ detail_invoice.unit_sale }} x
									...
									<tr style="margin-top: 10px" v-show="pos_settings.show_discount">
										...
										<td style="text-align: right" class="total">
											{{ invoice_pos.symbol }} {{ formatNumber(invoice_pos.sale.taxe, 2) }} ({{ formatNumber(invoice_pos.sale.tax_rate, 2) }} %)
										</td>
									</tr>
									...
									<tr v-show="invoice_pos.sale.paid_amount < invoice_pos.sale.GrandTotal">
										...
										<td style="text-align: right" class="total">
											{{ invoice_pos.symbol }} {{ parseFloat(invoice_pos.sale.GrandTotal - invoice_pos.sale.paid_amount).toFixed(2) }}
										</td>
									</tr>
								</tbody>
							</table>
						...
						<b-form @submit.prevent="Submit_Payment">
							...
							<b-row class="mt-4">
								...
								<b-col md="12" class="mt-3">
									<b-button variant="primary" type="submit" :disabled="paymentProcessing"><i class="i-Yes me-2 font-weight-bold"></i> {{ $t("submit") }}</b-button>
							...
							<b-row>
								...
								<b-col md="12" sm="12">
									<b-form-group :label="$t('Adress')">
										<textarea label="Adress" class="form-control" rows="4" v-model="client.adresse" :placeholder="$t('Adress')"></textarea>
									</b-form-group>
								</b-col>
				...
				<div class="pos-button-actions" style="display: flex; margin-top: 10px; bottom: 0px; margin-left: 29px; width: 100%; flex-wrap: wrap">
					<b-button style="width: auto; margin-bottom: 8px" @click="Reset_Pos()" variant="danger ripple btn-rounded mr-1">
						<i class="i-Power-2"></i>
						{{ $t("Reset") }}
					</b-button>

					<b-button style="width: auto; margin-bottom: 8px" @click="Submit_Pos()" variant="success ripple mr-1 btn-rounded">
						<i class="i-Checkout"></i>
						{{ $t("payNow") }}
					</b-button>

					<b-button style="width: auto; margin-bottom: 8px" @click="Submit_Draft()" :disabled="DraftProcessing" variant="primary ripple mr-1 btn-rounded">
						<i class="i-Sand-watch"></i>
						Draft
					</b-button>

					<b-button style="width: auto; margin-bottom: 8px" @click="Show_Draft_Sales()" variant="light ripple mr-1 btn-rounded">
...
export default {
	...
	computed: {
		...
		filteredCategories() {
			...
			return this.paginated_Category.filter((category) => category.name.toLowerCase().includes(this.search_category.toLowerCase()));
		},
		...
		filteredBrands() {
			...
			return this.paginated_Brands.filter((brand) => brand.name.toLowerCase().includes(this.search_brand.toLowerCase()));
		},
	...
	methods: {
			...
			axios
				...
				.then((response) => {
					if (response.data.success === true) {
						...
						this.makeToast("success", this.$t("Draft_Created_successfully"), this.$t("Success"));
						NProgress.done();
		...
		get_Draft_Sales(page) {
			...
			axios
				.get("get_draft_sales?page=" + page + "&limit=" + this.limit)
				.then((response) => {
			...
			this.$swal({
				...
				if (result.value) {
					...
					axios
						...
						.then(() => {
							this.$swal(this.$t("Delete.Deleted"), this.$t("Draft_Sale_Deleted"), "success");
						...
						.catch(() => {
							...
							this.$swal(this.$t("Delete.Failed"), this.$t("Delete.Therewassomethingwronge"), "warning");
						});
				}
			});
		},
		...
		Create_Client() {
			axios
				...
				.then((response) => {
					NProgress.done();
					this.makeToast("success", this.$t("Create.TitleCustomer"), this.$t("Success"));
		...
		Get_Client_Without_Paginate() {
			axios.get("get_clients_without_paginate").then(({ data }) => (this.clients = data));
		},
		...
		async processPayment() {
			...
			if (error) {
				...
				axios
					.post("pos-reservation/create_pos", {
						client_id: this.sale.client_id,
						warehouse_id: this.sale.warehouse_id,
						tax_rate: this.sale.tax_rate ? this.sale.tax_rate : 0,
						tax_net: this.sale.tax_net ? this.sale.tax_net : 0,
